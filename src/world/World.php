...
use pocketmine\event\block\ServerBlockBreakEvent;
...
class World implements ChunkManager{
	...
	public function useBreakOn(Vector3 $vector, Item &$item = null, ?Player $player = null, bool $createParticles = false, array &$returnedItems = []) : bool{
		...
		if($player !== null){
			...
			$ev->call();
			if($ev->isCancelled()){
				$this->addParticle($target->getPosition()->add(0.5, 0.5, 0.5), new BlockBreakParticle($target), [$player]);
				return false;
			}
			...
			$ev = new ServerBlockBreakEvent($target);
			$ev->call();
			if($ev->isCancelled()) {
				return false;
			}
	...
	public function useItemOn(Vector3 $vector, Item &$item, int $face, ?Vector3 $clickVector = null, ?Player $player = null, bool $playSound = false, array &$returnedItems = []) : bool{
		...
		if($player !== null){
			...
			$ev->call();
			if($ev->isCancelled()){
				$this->addSound($hand->getPosition(), new BlockPlaceSound($hand), [$player]);
				return false;
			}
		}
